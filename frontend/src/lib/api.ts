...
export const notificationsApi = {
  ...
  getMine: (
    ...
    api
      .get<PaginatedResponse<Notification>>("/notifications/mine", {
        params: { page, size },
      })
      .then((r) => {
  ...
  getCurrentMonthMyNotifications: (
    month: number,
    year: number
  ): Promise<PaginatedResponse<Notification>> =>
    api
  ...
  delete: (id: string) => apiClient.delete(`/notifications/${id}`), // <-- เปลี่ยน path ให้ตรงกับ backend
  ...
  updateStatus: async (
    ...
    const result = await api
      .patch<Notification>(`/notifications/${id}`, { status })
      .then((r) => r.data);
...
export const teamApi = {
  ...
  getAvailableEmployees: (teamId: string): Promise<any[]> =>
    api
      .get(`/teams/${teamId}/available-employees`)
      .then((r) => r.data.data || r.data)
      .catch(() => []),
};
