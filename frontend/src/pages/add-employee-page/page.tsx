...
import type { Team, TeamMember, PermissionLevel } from "@/components/types/team"
...
export default function AddEmployeePage() {
  ...
  const handleDeleteTeam = async () => {
    ...
    if (!confirm('คุณแน่ใจหรือไม่ที่จะลบทีมนี้? การกระทำนี้ไม่สามารถย้อนกลับได้')) {
      return;
    }
  ...
  const fetchTeams = async () => {
    try {
      ...
      setTeams(data)
      if (currentTeam) {
        const updatedCurrentTeam = data.find(team => team.id === currentTeam.id)
        if (updatedCurrentTeam && JSON.stringify(updatedCurrentTeam) !== JSON.stringify(currentTeam)) {
  ...
  const fetchEmployees = useCallback(async (page: number = 1) => {
    try {
      setIsLoading(true)
      const { data: response } = await employeesApi.list({
        page,
        size: pageSize,
        sortBy: 'firstName',
        sortOrder: 'asc'
      })      const employeesWithSelect = response.data.map(emp => {
        const employeeProfile = emp.employeeProfile || {};
        return {
          id: emp.id,
          ...
          name: employeeProfile.firstName && employeeProfile.lastName 
            ...
            : (employeeProfile.firstName || employeeProfile.lastName || 'ไม่ระบุชื่อ'),
          department: employeeProfile.position || 'ไม่ระบุตำแหน่ง'
      }))

      setEmployees(employeesWithSelect)
  ...
  // Filter employees based on search query and current team members
  useEffect(() => {
    // First filter out employees who are already in the current team
    const availableEmployees = employees.filter(emp => {
      if (!currentTeam) return true
      return !currentTeam.members.some(member => member.id === emp.id)
    })
  ...
  const handleRemoveLeader = async (keepAsMember: boolean) => {
    ...
    try {
      if (keepAsMember) {
        ...
        await teamsApi.removeMember(currentTeam.id, selectedLeaderToRemove.membershipId)
        await teamsApi.addMember(currentTeam.id, selectedLeaderToRemove.id, false)
      } else {
        // Remove completely
        await teamsApi.removeMember(currentTeam.id, selectedLeaderToRemove.membershipId)
      }
      ...
      toast.success(keepAsMember ? 'เปลี่ยนสถานะเป็นสมาชิกเรียบร้อยแล้ว' : 'ลบสมาชิกเรียบร้อยแล้ว')
    } catch (error) {
      console.error('Error updating team member:', error)
      toast.error(keepAsMember ? 'ไม่สามารถเปลี่ยนสถานะได้' : 'ไม่สามารถลบสมาชิกได้')
    }
  ...
  const handleUpdatePermission = async () => {
    ...
    try {
      ...
      await teamsApi.removeMember(currentTeam.id, selectedMemberForPermission.membershipId);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <TeamManagement
            ...
            setCurrentTeam={setCurrentTeam}            setIsCreateTeamDialogOpen={setIsCreateTeamDialogOpen}
