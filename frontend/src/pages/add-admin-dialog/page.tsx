...
import type { PermissionLevel } from "@/components/types/team"
...
export default function AddAdminPage() {
  ...
  const [selectedEmployee, setSelectedEmployee] = useState<(Employee & { selected?: boolean }) | null>(null)
  const [isLoading, setIsLoading] = useState(true)
  const [admins, setAdmins] = useState<(Employee & { permissionLevel: PermissionLevel })[]>([])
  ...
  const fetchEmployees = useCallback(
    async (page = 1) => {
      try {
        ...
        setEmployees((prev) => (page === 1 ? employeesWithSelect : [...prev, ...employeesWithSelect]))
  ...
  const handleOpenPermissionDialog = (employee: Employee & { selected?: boolean }) => {
